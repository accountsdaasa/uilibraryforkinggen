-- ================================================================= --
--     KING UI - FINAL MODERNIZED V18 (Design Refonte + Fluid Anim)
-- ================================================================= --
...
-- Thèmes modernisés (Couleurs plus profondes et contrastées)
local BUILTIN_THEMES = {
    ["Default"] = {
        WindowBG = Color3.fromRGB(20, 20, 25),      -- Plus sombre pour le look moderne
        SidebarBG = Color3.fromRGB(15, 15, 20),     -- Contraste subtil
        ItemBG = Color3.fromRGB(30, 30, 35),        -- Elements légèrement plus clairs
        Accent1 = Color3.fromRGB(180, 60, 230),     -- Purple
        Accent2 = Color3.fromRGB(60, 160, 210),     -- Blue
        ToggleON = Color3.fromRGB(180, 60, 230),    -- Match Accent1 par défaut
...
local function MakeDraggable(topbarobject, object, saveKey)
    ...
    UserInputService.InputChanged:Connect(function(input)
        if input == DragInput and Dragging then
            ...
            -- Utilisation de Tween pour le drag (optionnel, rend le drag un peu "élastique" ou direct)
...
function Library:ApplyTheme(themeName)
    ...
    for _, obj in pairs(Library.ThemeObjects.BgMain) do
        -- Modern: Solid or subtle gradient
...
function Library:Window(options)
    ...
    ScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling -- Better layering

    -- Shadow/Glow Effect (Procedural)
    ...
    ShadowHolder.Size = UDim2.new(0.45, 20, 0.65, 20)
    ShadowHolder.ZIndex = 0

    local ShadowImage = Instance.new("ImageLabel", ShadowHolder)
    ShadowImage.Image = "rbxassetid://6015897843" -- Generic shadow asset
    ...
    MainCorner.CornerRadius = UDim.new(0, 12) -- Slightly tighter modern corner
    ...
    local arConstraint = Instance.new("UIAspectRatioConstraint", ShadowHolder)
    ...
    TitleBar.Size = UDim2.new(1, 0, 0, 50) -- Slightly taller for clean look
    TitleBar.BackgroundColor3 = Theme.SidebarBG
    TitleBar.BackgroundTransparency = 1 -- Cleaner look without hard separation
    TitleBar.ZIndex = 2
    ...
    table.insert(Library.ThemeObjects.Strokes, TitleDivider) -- Use Stroke color for dividers
    ...
    MinIcon.Image = "rbxassetid://6031094678" -- Minus icon
    MinIcon.ImageColor3 = Theme.Text
    table.insert(Library.ThemeObjects.Texts, MinIcon) -- Treat icon as text color
    ...
    KBtn.Size = UDim2.new(0, 0, 0, 0) -- Hidden init
    ...
    function WindowAPI:Tab(name)
        ...
        -- Active Indicator (Little colored bar on left)
        ...
        function Elements:Slider(options)
            ...
            table.insert(Library.ThemeObjects.Texts, ValLabel) -- Update with theme, but override color below in logic
            ...
            KnobShadow.Color = Theme.AccentStart
            ...
            SliderBarBG.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    Dragging = true
                    Update(input)
                    Tween(Knob, {Size = UDim2.new(0, 18, 0, 18)}, 0.2) -- Pop effect
                end
            end)
            UserInputService.InputChanged:Connect(function(input)
        ...
        function Elements:Dropdown(options)
            ...
            function DropdownAPI:Update(newList)
                ...
                if Open then ToggleMenu() ToggleMenu() end -- Refresh if open
            end
            return DropdownAPI
        end
    ...
    -- // 6. AUTO CREDITS & SETTINGS (Keep logic, just update call style if needed)
