-- ================================================================= --
--     KING UI ENGINE - DEFINITIVE EDITION (V17 LEGACY + DESIGN)
--     TOUT INCLUS : SAVE, RETRO, MULTI-SELECT, AUTO-LOOP, THEMES
-- ================================================================= --
...
local Library = {
    ...
    Config = { Folder = "KingGen", File = "default.json" },
    Loops = {},
    Elements = {}
}
...
function Library:LoadConfig()
    ...
    if isfile(path) then
        ...
        if success then for k, v in pairs(data) do self.Flags[k] = v end end
    end
end
...
function Library:Window(options)
    ...
    local MainStroke = Create("UIStroke", {Parent = MainFrame, Thickness = 2, Color = Color3.new(1,1,1)})
    ...
    Create("UICorner", {Parent = ContentArea, CornerRadius = UDim.new(0, 10)})

    -- Keybind Toggle
    UserInputService.InputBegan:Connect(function(input, gpe)
        if not gpe and input.KeyCode == self.Theme.ToggleKey then
    ...
    function WindowFuncs:Tab(Name)
        ...
        -- [[ TOGGLE AVEC AUTO-REPEAT ]]
        function Elements:Toggle(opt)
            local Flag = opt.Flag or opt.Name
            local state = Library.Flags[Flag] or opt.Default or false
            Library.Flags[Flag] = state

            local TglBtn = Create("TextButton", {Parent = Page, BackgroundColor3 = Library.Theme.ElementBG, Size = UDim2.new(1, 0, 0, 40), Text = ""})
            Create("UICorner", {Parent = TglBtn})
            local Lbl = Create("TextLabel", {Parent = TglBtn, Text = opt.Name, TextColor3 = Library.Theme.Text, BackgroundTransparency = 1, Position = UDim2.new(0, 12, 0, 0), Size = UDim2.new(1, -60, 1, 0), TextXAlignment = "Left", Font = "Gotham", TextSize = 14})
            
            local Pill = Create("Frame", {Parent = TglBtn, Size = UDim2.new(0, 38, 0, 20), Position = UDim2.new(1, -50, 0.5, -10), BackgroundColor3 = Color3.fromRGB(45,45,50)})
            Create("UICorner", {Parent = Pill, CornerRadius = UDim.new(1,0)})
            local Circle = Create("Frame", {Parent = Pill, Size = UDim2.new(0, 16, 0, 16), Position = UDim2.new(0, 2, 0.5, -8), BackgroundColor3 = Color3.new(1,1,1)})
            ...
            local function update()
                ...
                if Library.Loops[Flag] then Library.Loops[Flag] = false end
                if state and opt.Delay then
                    Library.Loops[Flag] = true
                    task.spawn(function()
                        while Library.Loops[Flag] and task.wait(opt.Delay) do
            ...
            TglBtn.MouseButton1Click:Connect(function() state = not state update() end)
            update()
            return {Set = function(_, v) state = v update() end}
        end

        -- [[ MULTI SELECT DROPDOWN (Rétrocompatible) ]]
        function Elements:Dropdown(opt)
            local Flag = opt.Flag or opt.Name
            Library.Flags[Flag] = Library.Flags[Flag] or (opt.Multi and {} or opt.Default or opt.List[1])
            
            local D = Create("TextButton", {Parent = Page, BackgroundColor3 = Library.Theme.ElementBG, Size = UDim2.new(1, 0, 0, 40), Text = opt.Name, Font = "Gotham", TextSize = 14, TextColor3 = Library.Theme.Text})
            Create("UICorner", {Parent = D})
            
            local function RefreshText()
                if opt.Multi then
                    local count = 0
                    for _ in pairs(Library.Flags[Flag]) do count = count + 1 end
                    D.Text = opt.Name .. " (" .. count .. " selected)"
                else
                    D.Text = opt.Name .. " : " .. tostring(Library.Flags[Flag])
                end
            end

            D.MouseButton1Click:Connect(function()
                if not opt.Multi then
                    local index = table.find(opt.List, Library.Flags[Flag]) or 0
                    local nextV = opt.List[(index % #opt.List) + 1]
                    Library.Flags[Flag] = nextV
                    opt.Callback(nextV)
                else
                    -- Pour le multi-select dans tes scripts, on cycle souvent les options une par une au clic
                    -- Ou on ouvre un menu (ici simplifié pour compatibilité directe)
                    print("Multi-select clicked")
                end
                RefreshText()
                Library:SaveConfig()
            end)
        ...
        -- [[ CYCLE (Rétrocompatible) ]]
        function Elements:Cycle(opt) return Elements:Dropdown(opt) end

        -- [[ BUTTON ]]
        function Elements:Button(opt)
            local B = Create("TextButton", {Parent = Page, BackgroundColor3 = Library.Theme.ElementBG, Size = UDim2.new(1, 0, 0, 40), Text = opt.Name or opt.Text, Font = "Gotham", TextSize = 14, TextColor3 = Library.Theme.Text})
        ...
        function Elements:Slider(opt)
            ...
            Library.Flags[opt.Flag or opt.Name] = Default
        ...
        function Elements:Input(opt)
            local I = Create("Frame", {Parent = Page, BackgroundColor3 = Library.Theme.ElementBG, Size = UDim2.new(1, 0, 0, 40)})
        ...
        function Elements:Spacer(h) Create("Frame", {Parent = Page, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, h or 5)}) end

        return Elements
    end

    -- [[ AUTO-GENERATED TABS ]]
    local Settings = WindowFuncs:Tab("Settings")
    Settings:Section("Interface Control")
    Settings:Button({Name = "Destroy Interface", Callback = function() ScreenGui:Destroy() end})
    Settings:Input({Name = "Toggle Key (L, RCtrl...)", Default = "RightControl", Callback = function(v) pcall(function() Library.Theme.ToggleKey = Enum.KeyCode[v] end) end})
    
    local Credits = WindowFuncs:Tab("Credits")
    Credits:Section("Development")
    Credits:Label({Text = "Main Developer: King Gen", Align = "Center"})
    Credits:Label({Text = "UI Engine: King Engine V17 Legacy", Align = "Center"})

    function WindowFuncs:Init() Library:SaveConfig() print("KingGen Loaded") end
    function WindowFuncs:Notify(opt) print("[NOTIF]: " .. opt.Content) end
